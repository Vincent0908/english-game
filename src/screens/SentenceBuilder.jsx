...
function SentenceBuilder({ navigate, completeGame }) {
  ...
  useEffect(() => {
    ...
    setAnswer([]); setTimer(TIME_PER_QUESTION); setFeedback(null); setShowTip(false); setShowTranslation(false)
  }, [index])
  ...
  const goNext = () => setTimeout(() => { if (index + 1 >= TOTAL_QUESTIONS) setPhase('result'); else setIndex(i => i + 1) }, 1500)

  const handleTimeout = () => { setFeedback('timeout'); setResults(p => [...p, { answer: currentQ.answer, translation: currentQ.translation, isCorrect: false }]); goNext() }

  const checkAnswer = (cur) => {
    const ok = cur.map(w => w.word).join(' ') === currentQ.answer
    setFeedback(ok ? 'correct' : 'wrong'); if (ok) setScore(s => s + 10)
    setResults(p => [...p, { answer: currentQ.answer, translation: currentQ.translation, isCorrect: ok }]); goNext()
  }

  const pickWord = (wordObj) => {
    if (feedback) return
    const newAnswer = [...answer, wordObj]
    setWordBank(p => p.filter(w => w.id !== wordObj.id)); setAnswer(newAnswer)
    if (newAnswer.length === currentQ.words.length) checkAnswer(newAnswer)
  }

  const returnWord = (wordObj) => { if (feedback) return; setAnswer(p => p.filter(w => w.id !== wordObj.id)); setWordBank(p => [...p, wordObj]) }
  const handleClear = () => { if (feedback) return; setWordBank(p => [...p, ...answer]); setAnswer([]) }
    ...
    return (
      <div className="min-h-screen bg-game px-6 py-6 max-w-2xl mx-auto flex items-center">
        <div className="w-full bg-panel border border-dim rounded-3xl px-8 py-10 flex flex-col items-center gap-4 text-center">
          <p className="text-6xl">{score >= TOTAL_QUESTIONS * 7 ? '🏆' : score >= TOTAL_QUESTIONS * 4 ? '⭐' : '💪'}</p>
          <h2 className="font-quest text-gold text-2xl">Quest Complete!</h2>
          <p className="text-5xl font-extrabold text-slate-200">{score} <span className="text-lg text-slate-400">/ {TOTAL_QUESTIONS * 10} pts</span></p>
          ...
          <div className="w-full flex flex-col gap-2 max-h-72 overflow-y-auto mt-2">
            {results.map((r, i) => (
              <div key={i} className={`flex justify-between items-center bg-navy rounded-xl px-4 py-3 border gap-3 ${r.isCorrect ? 'border-emerald-500/30' : 'border-danger/30'}`}>
                <div className="text-left"><p className="font-bold text-sm text-slate-200">{r.answer}</p><p className="text-xs text-slate-400 mt-0.5">{r.translation}</p></div>
                <span className={`font-bold flex-shrink-0 ${r.isCorrect ? 'text-emerald-400' : 'text-danger'}`}>{r.isCorrect ? '✓' : '✗'}</span>
              </div>
            ))}
          </div>
          <button className="bg-gradient-to-br from-gold to-[#ff8c00] rounded-xl px-8 py-3.5 text-dark font-extrabold text-base mt-2 hover:-translate-y-0.5 transition-all" onClick={() => completeGame('sentence', score, xp)}>
            Back to Quest Map
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-game px-6 py-6 max-w-2xl mx-auto flex flex-col gap-5">
      ...
      <div className={`border rounded-2xl px-6 py-6 flex flex-col items-center gap-4 transition-all ${cardBorder}`}>
        ...
        <div className="w-full min-h-[72px] bg-dark rounded-xl border border-dashed border-dim flex items-center justify-center p-3">
          {answer.length === 0
            ...
            : <div className="flex flex-wrap gap-2 justify-center">
                {answer.map(w => (
                  <button key={w.id} className="bg-[#1e3a5f] border border-blue-500 text-slate-200 rounded-lg px-3.5 py-2 text-sm font-bold hover:bg-[#162d4a] hover:scale-95 transition-all" onClick={() => returnWord(w)}>{w.word}</button>
                ))}
              </div>
          }
        </div>
        ...
        {!feedback && (
          <div className="flex gap-2 flex-wrap justify-center">
            <button className="bg-transparent border border-dim text-slate-400 rounded-lg px-3 py-1.5 text-sm hover:text-slate-200 transition-colors" onClick={() => setShowTranslation(h => !h)}>🇮🇩 {showTranslation ? 'Hide' : 'Translation'}</button>
            <button className="bg-transparent border border-dim text-slate-400 rounded-lg px-3 py-1.5 text-sm hover:text-slate-200 transition-colors" onClick={() => setShowTip(h => !h)}>📐 {showTip ? 'Hide' : 'Grammar Tip'}</button>
          </div>
        )}
        {showTranslation && !feedback && <p className="text-amber-400 text-sm italic bg-amber-400/10 rounded-xl px-4 py-2 w-full text-center">🇮🇩 {currentQ.translation}</p>}
        {showTip        && !feedback && <p className="text-purple-400 text-sm italic bg-purple-400/10 rounded-xl px-4 py-2 w-full text-center">📐 {currentQ.tip}</p>}
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex gap-2.5 flex-wrap justify-center">
          {wordBank.map(w => (
            <button key={w.id} className="bg-[#1a2a1a] border-2 border-amber-400 text-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold hover:-translate-y-1 hover:shadow-[0_6px_16px_#f59e0b40] active:scale-95 transition-all disabled:opacity-40" onClick={() => pickWord(w)} disabled={!!feedback}>{w.word}</button>
          ))}
        </div>
      </div>

      <button className="bg-transparent border border-dim text-slate-400 rounded-xl py-3 text-sm hover:text-slate-200 transition-colors disabled:opacity-40" onClick={handleClear} disabled={!!feedback || !answer.length}>🗑️ Clear Sentence</button>
    </div>
  )
}
