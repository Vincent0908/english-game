...
function VocabularyMatch({ navigate, completeGame }) {
  ...
  useEffect(() => { setShuffledOptions(shuffle(currentQ.options)) }, [currentIndex])
  ...
  const handleAnswer = (option) => {
    ...
    setResults(prev => [...prev, { word: currentQ.word, correct: currentQ.correct, chosen: option, isCorrect }])
    setTimeout(() => {
      if (currentIndex + 1 >= TOTAL_QUESTIONS) setPhase('result')
      else { setCurrentIndex(i => i + 1); setSelected(null); setIsAnswered(false); setTimer(TIME_PER_QUESTION) }
    }, 1200)
  }

  if (phase === 'result') {
    const xp = Math.round((score / (TOTAL_QUESTIONS * 10)) * 100)
    return <GameResult score={score} total={TOTAL_QUESTIONS} xp={xp} onFinish={() => completeGame('vocab', score, xp)}>
      {results.map((r, i) => (
        <div key={i} className={`flex justify-between items-center bg-navy rounded-xl px-4 py-2.5 border text-sm gap-3 flex-wrap ${r.isCorrect ? 'border-emerald-500/30' : 'border-danger/30'}`}>
          ...
          <span className={r.isCorrect ? 'text-emerald-400' : 'text-danger'}>{r.isCorrect ? '✓' : '✗'} {r.correct}</span>
        </div>
      ))}
    </GameResult>
  }
  ...
  return (
    <div className="min-h-screen bg-game px-6 py-6 max-w-2xl mx-auto flex flex-col gap-5">
      <GameHeader title="🔤 Vocabulary Match" index={currentIndex} total={TOTAL_QUESTIONS} score={score} progress={currentIndex / TOTAL_QUESTIONS * 100} barColor="from-blue-500 to-teal" navigate={navigate} />
      ...
      <div className="bg-panel border border-dim rounded-2xl px-8 py-8 text-center" style={{ boxShadow: '0 0 40px #3b82f620' }}>
        ...
        <p className="font-quest text-3xl text-slate-200" style={{ textShadow: '0 0 20px #ffffff30' }}>{currentQ.word}</p>
      </div>
...
function GameHeader({ title, index, total, score, progress, barColor, navigate }) {
  return (
    <div className="flex justify-between items-center gap-3 bg-panel rounded-2xl px-5 py-4 border border-dim animate-fadeInUp">
      <button className="bg-transparent border border-dim text-slate-400 rounded-lg px-3 py-2 text-sm hover:text-slate-200 transition-colors" onClick={() => navigate('select')}>← Back</button>
      <div className="flex-1 text-center">
        <p className="text-slate-200 font-extrabold mb-2">{title}</p>
        <div className="bg-navy rounded-full h-1.5 overflow-hidden">
          <div className={`h-full rounded-full bg-gradient-to-r ${barColor} transition-all duration-300`} style={{ width: `${progress}%` }} />
        </div>
...
function GameResult({ score, total, xp, onFinish, children }) {
  ...
  return (
    <div className="min-h-screen bg-game px-6 py-6 max-w-2xl mx-auto flex items-center">
      <div className="w-full bg-panel border border-dim rounded-3xl px-8 py-10 flex flex-col items-center gap-4 text-center">
        ...
        <p className="text-5xl font-extrabold text-slate-200">{score} <span className="text-lg text-slate-400">/ {total * 10} pts</span></p>
        <p className="text-gold font-bold">+{xp} XP earned!</p>
        <div className="w-full flex flex-col gap-2 max-h-64 overflow-y-auto mt-2">{children}</div>
        <button className="bg-gradient-to-br from-gold to-[#ff8c00] rounded-xl px-8 py-3.5 text-dark font-extrabold text-base mt-2 hover:-translate-y-0.5 transition-all" onClick={onFinish}>
          Back to Quest Map
        </button>
      </div>
    </div>
