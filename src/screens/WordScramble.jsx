      ...
      <div className={`border rounded-2xl px-7 py-7 text-center flex flex-col items-center gap-4 transition-all ${cardBorder}`}>
              ...
              <div
                ...
                className={`w-11 h-12 rounded-xl border-2 flex items-center justify-center transition-all
                  ${isRevealed ? 'bg-purple-900/60 border-purple-500 cursor-default'
                    : letter ? 'bg-[#1a3a5c] border-blue-500 cursor-pointer'
                    : 'bg-navy border-dim cursor-default'}`}
                onClick={() => returnLetter(i)}
              >
                <span className={`font-extrabold text-xl ${isRevealed ? 'text-purple-400' : 'text-slate-200'}`}>
                  {letter ? letter.char : ''}
                </span>
              </div>
            )
        ...
        {!feedback && (
          <div className="flex gap-2 flex-wrap justify-center">
            ...
            <button
              className="bg-purple-500/10 border border-purple-500/30 text-purple-400 rounded-lg px-3 py-1.5 text-sm hover:bg-purple-500/20 transition-colors"
        ...
        {showHint && !feedback && (
          <p className="text-amber-400 text-sm italic bg-amber-400/10 rounded-xl px-4 py-2">💡 {currentQ.hint}</p>
        )}
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex gap-2.5 flex-wrap justify-center">
          {scrambled.map(letter => (
            <button
              key={letter.id}
              className="w-12 h-14 rounded-xl bg-[#1a3a5c] border-2 border-blue-500 text-slate-200 font-extrabold text-xl hover:-translate-y-1 hover:scale-105 hover:shadow-[0_6px_16px_#3b82f640] active:scale-95 transition-all disabled:opacity-40"
